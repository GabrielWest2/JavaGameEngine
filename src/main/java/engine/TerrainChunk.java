...
public class TerrainChunk {
    ...
    public TerrainChunk(int chunkX, int chunkY, int lod){
        transformationMatrix = MatrixBuilder.createTransformationMatrix(
            new Vector3f(chunkX * totalWidth, 0, chunkY * totalWidth),
            new Vector3f(0.0f),
            new Vector3f(1.0f)
            );
    ...
    private void createTerrain(int chunkX, int chunkY, int LOD)
            throws InvalidTerrainChunkError {
        if(chunkX < 0 ||
            chunkY < 0 ||
            chunkX > terrainChunkWidth ||
            chunkY > terrainChunkWidth
        ){
            throw new InvalidTerrainChunkError
        ...
        for (int y = 0; y < vertexWidth; y++) {
            for (int x = 0; x < vertexWidth; x++, i += 3, j += 2) {
                ...
                vertices[i+ 1] = getTextureHeight((float) x / vertexWidth,
                        (float) y / vertexWidth);
                ...
                Vector3f normal = calculateNormal(
                        (float) x / vertexWidth,
                        (float) y / vertexWidth,
                        1f / vertexWidth * heightmap.getWidth());
                normals[i] = normal.x;
        ...
        model = ModelCreator.loadToTerrainVAO(
                vertices,
                triangles,
                textureCoords,
                normals,
    ...
    public float getTextureHeight(float x, float y){
        int pixel = heightmap.getRGB(
                (int) Math.round((0 + x) * heightmap.getWidth()),
                Math.round((y * heightmap.getHeight()))
        );
